...
namespace Zentlix\MainBundle\Domain\Route\Specification;
        ...
        foreach ($routes as $route) {
            if($route->getSite()->getId() === $value[1]) {
                if($route->getCleanUrl() === Route::cleanUrl($value[0])) {
                    throw new NonUniqueResultException(sprintf($this->translator->trans('zentlix_main.route.already_exist'), $value[0]));
                }
            }
        }
